...
public class PtGen {
	...
	private static int code, indexVar = 0, identAffect, varType, varCat, varInfo, nbParam, paramInTabSymb = -1, varConstanteInProc = 0, nombreVarRef = 0;
	private static String nomProc = null;
		...
		switch (numGen) {
			...
			case 5:	if (bc == 1) {
						...
						if(presentIdent(bc) == 0) { 
							placeIdent(UtilLex.numId, VARGLOBALE, tCour, indexVar); 
							indexVar++;
							// mise a jour de tailleGlobaux dans le descripteur
			...
			case 6: // on produit RESERVER dans le cas d'un programme				
					if(desc.getUnite().equals("programme") && bc == 1) {
			...
			case 32: // on verifie su notre ident est present dans tabSymb
					 ...
					 if(identAffect == 0) UtilLex.messErr("L'identifiant " + UtilLex.repId(UtilLex.numId) + " n'existe pas dans tabSymb");
					 	...
					 	if(tabSymb[identAffect].categorie == CONSTANTE  || tabSymb[identAffect].categorie == PARAMFIXE)
					 		UtilLex.messErr("Affection impossible, nous sommes en presence d'une constante ou d'un parametre fixe");
			...
			case 34: // on verifie si notre ident est present dans tabSymb 
					 ...
					 if(index1 == 0) UtilLex.messErr("L'identifiant " + UtilLex.repId(code) + "n'existe pas dans tabSymb");
		 				...
		 				if(tabSymb[index1].categorie == CONSTANTE || tabSymb[index1].categorie == PARAMFIXE)
		 					UtilLex.messErr("Affectation impossible, nous sommes en presence d'une constante ou d'un parametre fixe");
		 				else {
		 					// on verifie le type
			...
			// aucune instruction aut apres COND
			case 41: ListIterator<Integer> it2 = suiteBincond.listIterator(suiteBincond.size());
			 		 // on parcourt notre liste pour mettre a jour les differents BINCOND
			 		 while(it2.hasPrevious()) {
			 			int bincond = it2.previous(); 
			 			if(bincond != 0) {
			 				...
			 				// on supprime le zero de notre liste
			 				it2.remove();
	        ...
	        case 51: // depilement et mise a jour du bincond à la fin de la procédure
	        		 if(desc.getUnite().equals("programme")) { po.modifier(pileRep.depiler(), po.getIpo()+1); } 
	                 break;
	        ...
	        case 52: // declaration de la procedure
	        	     nomProc = UtilLex.repId(UtilLex.numId);
	        	 	 if(presentIdent(1) == 0){
	        	 		...
	        	 		// mettre a jour tabDef [indiceIpo]
		                int isPresent = desc.presentDef(nomProc);
		                if(isPresent != 0) desc.modifDefAdPo(isPresent, po.getIpo()+1);
	        ...
	        case 55: // a la fin de la declaration de la procedure
	        		...
	        		// mettre a jour tabDef [nbParam]
	                int isPresent = desc.presentDef(nomProc);
	                if(isPresent != 0) desc.modifDefNbParam(isPresent, nbParam);
	        ...
	        case 58: if(varType == NEUTRE && varCat == PROC) {
	    				...
	    				if (tabSymb[identAffect+1].categorie == REF)
	        ...
	        /** CODE COMPILATION SEPAREE **/
	        		
	        case 70: // ajout du module dans le descripteur
	        		 desc.setUnite("module");	 
	        		 break;
	        		 
	        case 71: // ajout du programme dans le descripteur
	        		 desc.setUnite("programme");
	        		 break;
	        
	        case 72: // definition de procedure ajout dans tabDef	        		 
	        		 desc.ajoutDef(UtilLex.repId(UtilLex.numId));	        		 
	        		 break;
	        		 
	        case 73: // reference sur procedure ajout dans tabRef
	        		 ...
	        		 if(presentIdent(1) == 0) {
						...
						UtilLex.messErr("La procedure " + UtilLex.repId(UtilLex.numId) + " existe deja.");	
	        		 break;
	        		 
	        case 74: // insertion dans tabSymb des parametres fixe de la procedure reference
   			 		 placeIdent(-1, PARAMFIXE, tCour, nombreVarRef);
   			 		 nombreVarRef ++;
	        		 break;
	        		
	        case 75: // insertion dans tabSymb des parametres mod de la procedure reference
	        		 placeIdent(-1, PARAMMOD, tCour, nombreVarRef);
	        		 nombreVarRef ++;
	        		 break;
	        		 
	        case 76: // mise a jour de tabSymb
	        		 if(tabSymb[bc-1].categorie == REF) {
	        			 tabSymb[bc-1].info = nombreVarRef;
	        		 }	        		 
	        		 // mise a jour de tabRef [nbvarRef]
	        		 int presentRef = desc.presentRef(UtilLex.repId(UtilLex.numId));
	        		 if(presentRef != 0) {
	        			 desc.modifRefNbParam(presentRef, nombreVarRef);
	        		 }
	        		 nombreVarRef = 0;	 
	        		 bc = 1;
	        		 break;
			...
			// produit arret et creation du code Mapile [Cas de programme et Module] 				
			case 200: if(desc.getUnite().equals("programme")) { po.produire(ARRET); }
			
					  //afftabSymb();
					  // mise a jour de tailleCode
			...
			case 201: // Mnemo.creerFichier(po.getIpo(), po, UtilLex.nomSource);
