...
unitprog
  : 'programme' ident ':' { PtGen.pt(71); }
     declarations  
     corps { System.out.println("succes, arret de la compilation "); { PtGen.pt(200); } }
  ;
  
unitmodule
  : 'module' ident ':' { PtGen.pt(70); }
     declarations { System.out.println("succes, arret de la compilation "); { PtGen.pt(200); } }
  ;
  
declarations
  : partiedef? partieref? consts? vars? { PtGen.pt(6); } decprocs? 
  ;
  
partiedef
  : 'def' ident { PtGen.pt(72); } (',' ident { PtGen.pt(72); } )* ptvg
  ;
  
partieref: 'ref' specif { PtGen.pt(76); }  (',' specif { PtGen.pt(76); } )* ptvg
  ;
  
specif  : ident { PtGen.pt(73); } ( 'fixe' '(' type { PtGen.pt(74); } ( ',' type { PtGen.pt(74); } )* ')' )?
                 ( 'mod'  '(' type { PtGen.pt(75); } ( ',' type { PtGen.pt(75); } )* ')' )?
  ;
  
consts  : 'const' ( ident { PtGen.pt(1); } '=' valeur { PtGen.pt(2); } ptvg )+ 
  ;
  
vars  : 'var' ( type ident { PtGen.pt(1); PtGen.pt(5); } ( ','  ident { PtGen.pt(1); PtGen.pt(5); } )* ptvg )+
  ;
...
decprocs: { PtGen.pt(50); } (decproc ptvg)+ { PtGen.pt(51); }
  ;
  
decproc :  'proc'  ident { PtGen.pt(52); }  parfixe? parmod? { PtGen.pt(55); } consts? vars? { PtGen.pt(6); } corps { PtGen.pt(56); } 
  ;
...
pf  : type ident { PtGen.pt(53); }  ( ',' ident { PtGen.pt(53); }  )*  
  ;

parmod  : 'mod' '(' pm ( ';' pm)* ')'
  ;
  
pm  : type ident { PtGen.pt(54); }  ( ',' ident { PtGen.pt(54); }  )*
  ;
...
effmods :'(' (ident { PtGen.pt(57); } (',' ident { PtGen.pt(57); }  )*)? ')'
  ; 
