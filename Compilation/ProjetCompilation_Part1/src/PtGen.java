...
public class PtGen {
	...
	private static int code, indexVar = 0, identAffect, varType, varCat;
	...
	public static void pt(int numGen) {
		...
		switch (numGen) {
			...
			// lecture d'un ident de
			case 5: if(presentIdent(1) == 0) { placeIdent(UtilLex.numId, VARGLOBALE, tCour, indexVar); indexVar++; }
			...
			case 32: // on verifie su notre ident est present dans tabSymb
					 ...
					 if(identAffect== 0) UtilLex.messErr("L'identifiant " + UtilLex.repId(UtilLex.numId) + " n'existe pas dans tabSymb");
					 	...
					 	if(tabSymb[identAffect].categorie == CONSTANTE)
			...
			case 33: if(varType == ENT) verifEnt();
					 ...
					 po.produire(tabSymb[identAffect].info);			
	 				 break;
			...
			case 34: // on verifie si notre ident est present dans tabSymb 
					 ...
					 if(index1== 0) UtilLex.messErr("L'identifiant " + UtilLex.repId(code) + " n'existe pas dans tabSymb");
					 	//afftabSymb();
					  	// on verifie si le type de la categorie :: si CONSTANTE erreur
		 				if(tabSymb[index1].categorie == CONSTANTE)
		 					UtilLex.messErr("Affectation impossible, nous sommes en presence d'une constante");
		 				else
		 					// on verifie le type
		 					switch (tabSymb[index1].type) {
		 						// cas d'un entier
		 						case ENT: po.produire(LIRENT);
		 							      po.produire(AFFECTERG);
		 							      po.produire(tabSymb[index1].info);
		 							      break;
		 							      
		 					    // cas d'un boolean
		 						case BOOL: po.produire(LIREBOOL);
							      		   po.produire(AFFECTERG);
							      		   po.produire(tabSymb[index1].info);
							      		   break;
			...
			// fin du COND
			case 41: ListIterator<Integer> it = suiteBincond.listIterator(suiteBincond.size());
